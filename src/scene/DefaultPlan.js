...
export function buildDefaultPlan(W, H) {
  ...
  // ===== 院界（域中央 48×54：W=96/H=80 时 x 24..71，y 13..66）=====
  const midX = (W / 2) | 0;                        // 48
  const x0 = midX - 24, x1 = midX + 23;            // 院墙东西 24..71
  const y0 = 13, y1 = 66;                          // 院墙南北 13..66
  hline(x0, x1, y0);                               // 北院墙
  hline(x0, x1, y1);                               // 南院墙
  vline(x0, y0, y1);                               // 西院墙
  vline(x1, y0, y1);                               // 东院墙

  // ===== 北正房（三间：堂屋居中 + 东西次间），j 14..28 =====
  hline(x0 + 1, x1 - 1, 28);                       // 正房南墙（前檐墙）
  const zhTangX0 = midX - 6, zhTangX1 = midX + 5;  // 堂屋 42..53（宽12格）
  vline(zhTangX0 - 1, 15, 27);                     // 堂屋东隔墙（东侧次间分界）
  vline(zhTangX1 + 1, 15, 27);                     // 堂屋西隔墙
  openRange(zhTangX0, zhTangX1, 28);               // 堂屋开门面庭（敞厅）
  open(x0 + 6, 28); open(x0 + 7, 28);              // 东次间门
  open(x1 - 7, 28); open(x1 - 6, 28);              // 西次间门

  // ===== 东西厢房（竖长条），j 32..52 =====
  hline(x0 + 1, x0 + 8, 32); hline(x0 + 1, x0 + 8, 52);   // 西厢房南北墙
  vline(x0 + 8, 33, 51);
  hline(x1 - 8, x1 - 1, 32); hline(x1 - 8, x1 - 1, 52);   // 东厢房南北墙
  vline(x1 - 8, 33, 51);
  open(x0 + 4, 32); open(x0 + 4, 52);              // 西厢房南北门（穿堂）
  open(x1 - 4, 32); open(x1 - 4, 52);              // 东厢房南北门

  // ===== 南倒座（门房），j 56..65，东南巽位留大门 =====
  hline(x0 + 1, x1 - 1, 56);                       // 倒座北墙
  vline(midX - 10, 57, 65);                        // 倒座内隔（杂物/门房两区）
  openRange(x1 - 6, x1 - 2, y1);                   // 东南巽位大门（65..69 开五格）
  open(x1 - 6, 56); open(x1 - 5, 56);              // 门道通倒座北
