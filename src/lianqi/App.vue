<template>
  <div class="game-container-wrapper" draggable="true">
    <!-- 返回星空门户：与命理/堪舆「← 返回」同款文案；存档 localStorage 自动持久化，跳出不丢进度 -->
    <a class="portal-back" href="index.html" title="返回星空门户">← 返回</a>
    <div :class="['game-container', { dark: player.dark }]">
      <router-view v-slot="{ Component }">
        <keep-alive :include="['MapExploration']">
...
<style>
  * {
  ...
  /* 深色模式下的返回钮：墨底灰字，不刺眼 */
  html.dark .portal-back {
    color: rgba(207, 211, 220, 0.72);
    background: rgba(20, 20, 20, 0.55);
    border-color: rgba(207, 211, 220, 0.22);
  }

  html.dark .portal-back:hover {
    color: #e5eaf3;
