  ...
  <div class="index">
          ...
          <el-tabs v-model="inventoryActive" :stretch="true">
            <el-tab-pane label="装备" name="equipment">
              ...
              <el-button
                ...
                @click="decomposeByType"
              >
                一键分解{{ genre[equipmentActive] }}
              </el-button>
            ...
            <el-tab-pane label="道侣" name="wife">
              <div class="inventory-content">
                <template v-for="(item, index) in allWives" :key="index">
                  <tag
                    class="inventory-item"
                    :class="{ 'wife-unmarried': !item.married }"
                    @click="item.married ? wifeItemInfo(getWife(item.name)) : npcWifeInfo(item)"
                  >
                    <img
                      v-if="assetUrl('wife', wifeTexKey[item.name])"
                      class="wife-avatar"
                      :src="assetUrl('wife', wifeTexKey[item.name])"
                      alt=""
                    />
                    {{ item.name }}
                    <span class="wife-status">{{ item.married ? '♥道侣' : `♥${item.favorability}` }}</span>
                  </tag>
                </template>
              </div>
            </el-tab-pane>
    ...
    <el-drawer :title="player.wife?.name" v-model="wifeItemShow" direction="rtl" class="strengthen">
      <div class="strengthen-box">
        <div class="wife-figure-box" v-if="player.wife?.name && assetUrl('wife', wifeTexKey[player.wife.name])">
          <img class="wife-figure-lg" :src="assetUrl('wife', wifeTexKey[player.wife.name])" alt="" />
        </div>
        <div class="attributes">
          <div class="attribute-box">
...
<script setup>
  ...
  // 道侣像素形象
  import { assetUrl, wifeTexKey } from '@/game/textures'
  ...
  // 一键分解当前分类装备 (神兵/护甲/灵宝/法器, 跳过锁定)
  const decomposeByType = () => {
    const type = equipmentActive.value
    const typeName = genre[type]
    const selling = player.value.inventory.filter(item => item.type === type && !item.lock)
    // 没有可分解的装备
    if (!selling.length) {
      gameNotifys({ title: '装备分解提示', message: `${typeName}分类下没有可分解的装备(锁定装备不会分解)` })
      return
    }
    // 折算规则与批量分解一致: 等级总和 -> 炼器石, 件数 -> 灵石
    ...
    ElMessageBox.confirm(
      `将分解${typeName}分类下所有非锁定装备 ${selling.length} 件, 获得 ${strengtheningStoneTotal} 个炼器石和 ${selling.length} 个灵石, 锁定装备不受影响`,
      '一键分解',
      { center: true, cancelButtonText: '取消', confirmButtonText: '确认分解' }
    )
      .then(() => {
        player.value.props.money += selling.length
        player.value.props.strengtheningStone += strengtheningStoneTotal
        const ids = new Set(selling.map(i => i.id))
        player.value.inventory = player.value.inventory.filter(i => !ids.has(i.id))
        gameNotifys({
          title: '装备分解提示',
          message: `${typeName}分类分解完成, 你获得了${strengtheningStoneTotal}个炼器石和${selling.length}个灵石`
        })
      })
      .catch(() => {})
  }
  ...
  // 全部道侣人选 (地图十位仙子, 含未结缘): 已结缘标道侣, 未结缘显示好感度
  const allWives = computed(() => {
    const married = new Set(player.value.wifes.map(w => w.name))
    const list = (player.value.npcs || []).map(n => ({
      name: n.name,
      married: married.has(n.name),
      favorability: n.favorability || 0
    }))
    // 兜底: 结缘了但不在地图 NPC 列表的数据
    const known = new Set(list.map(n => n.name))
    player.value.wifes.forEach(w => {
      if (!known.has(w.name)) list.push({ name: w.name, married: true, favorability: 1000 })
    })
    return list
  })

  // 按名字取已结缘道侣数据
  const getWife = name => player.value.wifes.find(w => w.name === name)

  // 未结缘仙子信息 (好感度 + 结缘指引)
  const npcWifeInfo = item => {
    const wifeImg = assetUrl('wife', wifeTexKey[item.name])
    const imgHtml = wifeImg ? `<img class="wife-figure" src="${wifeImg}" alt="" />` : ''
    const ready = item.favorability >= 1000
    ElMessageBox.confirm(
      `<div class="monsterinfo">${imgHtml}
      <div class="monsterinfo-box">
      <p>好感度: ${item.favorability} / 1000</p>
      <p>状态: ${ready ? '<span style="color:#67C23A">可结缘</span>' : '未结缘'}</p>
      <p style="font-size:12px;color:#909399">前往秘境地图找到她, 赠送礼物提升好感度</p>
      </div></div>`,
      item.name,
      { center: true, showCancelButton: false, confirmButtonText: '知道了', dangerouslyUseHTMLString: true }
    ).catch(() => {})
  }
  ...
  const wifeItemInfo = item => {
    // 道侣像素形象 (有素材时在弹窗顶部展示)
    const wifeImg = assetUrl('wife', wifeTexKey[item.name])
    const imgHtml = wifeImg
      ? `<img class="wife-figure" src="${wifeImg}" alt="" />`
      : ''
